<?php
// $Id: abstract.php 15788 2015-10-21 08:01:56Z guohua $
 ...
 * @version $Id: abstract.php 15788 2015-10-21 08:01:56Z guohua $
 * @package database
 */
...
abstract class QDB_Adapter_Abstract
{
    ...
    function qid($name, $alias = null, $as = null)
    {
        ...
        else
        {
            ...
            foreach ($arr as $offset => $name)
            {
                ...
                elseif(strtoupper($name) == 'NULL') //兼容 order by null 用法
                {
                    $arr = array($offset => $name);
                    break;
                }
                else
                {
        ...
        if ($alias)
        {
            $rs = "{$name} {$as} " . $this->identifier($alias);
        }
        else
        {
            $rs = $name;
        ...
        //临时处理: 聚合查询统计字段的BUG
        $stat_funcs = array("COUNT", "SUM", "DISTINCT", "AVG", "MAX", "MIN", "EXPR");
        $stat_funcs_yes = false;
        foreach($stat_funcs as $k => $func)
        {
            $mode = "/{$func}\(/is";
            if(preg_match($mode, $rs) || stripos($rs, $func . ":")) 
            {
                $stat_funcs_yes = true;
                break;
            }
    ...
    function qinto($sql, array $params = null, $param_style = null, $return_parameters_count = false)
    {
        ...
        {
            case QDB::PARAM_QM:
                //$params参数统一为数组, 并且降级为2级数组
                $params = Helper_System::degradeArray($params, 2);
                $str = $this->doReplaceForPlaceHolder($sql, $params);
     ...
     *
     * @额外案例:
     * - $output = PHP_Object::doReplaceForPlaceHolder("k1 in(?) and k2 > ?", array("v1","v2"), "v3");
     * - $output = PHP_Object::doReplaceForPlaceHolder("k1 in(?) and k2 > ?", array(array("v1","v2"), "v3"));
     * - $output = PHP_Object::doReplaceForPlaceHolder("k1 in(?) and k2 > ?", array(array("v1","v2"), "v3"), 8);
     * - $output = PHP_Object::doReplaceForPlaceHolder("k1 in(?) and k2 > ?", array(array(array("v1","v2"))), "v3");
     * - $output = PHP_Object::doReplaceForPlaceHolder("k1 in(?) and k2 > ?", array(array(array(array("v1","v2")))), "v3");
     * - $output = PHP_Object::doReplaceForPlaceHolder("k1 in(?) and k2 = ?", 1, 2, 3);
     * - $output = PHP_Object::doReplaceForPlaceHolder("k1 = ? and k2 = ?", 1, 2, 3);
     * - $output = PHP_Object::doReplaceForPlaceHolder("k1 = ? and k2 = ? and k3 > ?", 1, 2, 3);
     * - $output = PHP_Object::doReplaceForPlaceHolder("k1 = ? and k2 = ?", array(1, 2), 3);
     * - $output = PHP_Object::doReplaceForPlaceHolder("k1 in(?) and k2 > ? and k3 in(?)", array(1, 2), 3, array(4,5));
     * - $output = PHP_Object::doReplaceForPlaceHolder("k1 in(?) and k2 > ? and k3 in(?)", array(array(array(1,2), 3, array(4,5))));
     *
     * @return  string
     */
    public function doReplaceForPlaceHolder()
    {
        $subject = '';
        $args = func_get_args();
        ...
        if(!empty($matches) && is_array($matches))
        {
            foreach($matches as $k => $v)
            {
                ...
                //如果不包含关键词IN
                if($find_in_keyword === false && Helper_System::getArrayDepth($replace) >= 2) 
                {
                    is_array($replace[0]) && $replace = $replace[0];
                }

                if(!isset($replace[1]) && Helper_System::getArrayDepth($replace) >= 2) 
                {
                    $replace = $replace[0];
                }
                ...
                {
                    $replace[$k] =  Helper_System::getArrayDepth($replace[$k]) == 1 
